...
public partial class PolarTransformEntity : Node2D
{
    #region 组件
    [Notify,Export] public PolarCoordinateComp PolarCoordinateComp { get => _polarCoordinateComp.Get(); set => _polarCoordinateComp.Set(value); }
    ...
    {
        PolarCoordinateComp.PositionChanged += OnTransformChanged;
        ...
        PolarCoordinateCompChanged += RespondAll;
    ...
    {
        var pl = PolarCoordinateComp.Position;
        var t = Transform2D.Identity.Transform2DAs3DDeg(
            CompleteRotationComp.Rotation,
            new Polar2(pl.R,pl.T.DegToRad()).ToCartesian() * GlobalUnit.UNIT_LENGTH,
            ScaleComp.Scale
        );
